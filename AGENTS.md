...
- `docs/` — 生成的网站（GitHub Pages 部署目录）：`index.html`、各分类页、`plugins.json`、`sitemap.xml`、`feed.xml`。自动生成，切勿手动编辑。
...
- `images/` — 品牌素材目录：`qr-code.jpg`（公众号二维码原图）、`logo.png`（由二维码转换的站点 favicon / masthead logo）。修改品牌图后重新运行 `node scripts/build-site.mjs` 并再次复制 `images/logo.png` 到 `docs/logo.png`。
